...
- Latest implementation commit: `8ec08d0 feat: read libsvn file details`
...
- Continue the real `svn-libsvn` backend beyond native version, RA repository metadata, RA log metadata, and changed-file content/property reads; remaining backend work includes directory-copy materialization, full replay/editor integration, auth/config support, remote service validation, and deeper libsvn error/session handling.
...
- With `VCPKG_ROOT=E:\vcpkg`, `VCPKG_DEFAULT_TRIPLET=x64-windows`, `VCPKGRS_DYNAMIC=1`, and `PATH` including `E:\vcpkg\installed\x64-windows\bin`: `cargo test -p git-svn-rs-core --features svn-libsvn --test libsvn_backend linked_backend_reads_log_metadata_and_changed_paths -- --nocapture` (covers changed-path metadata, file content, and `svn:executable`/`svn:special` file properties)
...
1. Continue Phase 4/7 production backend work in the configured vcpkg/libsvn environment, starting with native replay/editor integration and directory-copy materialization.
